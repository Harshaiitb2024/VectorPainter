...
class StrokeTextStylePipeline(ModelState):
    ...
    def painterly_rendering(self, text_prompt):
        ...
        # load renderer
        renderer = self.load_render(inputs, style_img=None)
        img = renderer.init_image(random=True)
        ...
        with tqdm(initial=self.step, total=total_iter, disable=not self.accelerator.is_main_process) as pbar:
            while self.step < total_iter:
                ...
                # total loss
                loss = sds_loss + total_visual_loss + l_tvd + l_percep_content
